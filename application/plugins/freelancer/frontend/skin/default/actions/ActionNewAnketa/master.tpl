...
{block 'layout_content' append}
    ...
    <form action="" method="post">
        ...
        {component 'field' template='geo'
            ...
            place     = $oGeoTarget} 
        {*ФИО*}<br>
        ФИО: <input type="text" name="name" value="{$sName}"><br>
        Подробнее об услугах: <textarea name='text_about'>{$sTextAbout}</textarea><br>
        Цена: <input type="text" size="15" name="price" value="{$sPrice}"/>
        {component 'field' template='select'
            name          = 'price_metod'
            label         = 'За'
            classes       = 'js-blog-add-field-type'
            inputClasses  = 'ls-width-200 js-blog-add-type'
            selectedValue = $sPriceMetod
            items = $aPriceSelector}
