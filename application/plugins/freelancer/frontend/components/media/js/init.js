...
(function($) {
    ...
    $.widget( "livestreet.lsUserMedia", $.livestreet.lsComponent, {
        /**
         * Дефолтные опции
         */
        options: {
            ...
            urls: {
                // Вставка файла
                insert: aRouter.freelancer + 'ajax/media/submit-insert/',
                // Вставка фотосета
                photoset:  aRouter.freelancer + 'ajax/media/submit-insert/',
                // Загрузка файла по ссылке
                url_upload: aRouter.freelancer + 'ajax/media/upload-link/',
        ...
        insertFiles: function( url, params, files ) {
            ...
            this._load( url, $.extend( true, {}, { ids: ids, size: '100x100crop' }, params || {} ), function( response ) {
                this.appendFiles(response.aMedia);
                this.element.lsModal( 'hide' );
            });
        },
        
        appendFiles:function(aMedia){
            var html = $(aMedia.html);
            html.find('.but_close').click(function(){
                $('input[value="'+$(this).attr('id')+'"]').remove();
                $(this).parent().remove();
                return false;
            });
            $('.media_items').append(html);
            var form = $('form');
            var i = $('.attach_media').length;
            aMedia.ids.forEach(function (val) {
                i++;
                var input = $("<input>").attr("type", "hidden").attr("name", "media["+i+"]").attr("class", "attach_media").val(val);
                form.append(input);                
            });
        },
...
jQuery(function ($) {
    //$('.order-media-mymodal').lsModal();
    //alert('media')
    $('.user-media-mymodal-toggle').lsModalToggle();
    $('#media_modal_user').lsUserMedia();
    $('.js-media-info-block-image-options').remove();
    $('.js-media-info-block[data-type="photoset"]').remove();
    $('.js-media-url-submit-insert').remove();
