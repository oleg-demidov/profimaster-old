...
{if $oUserCurrent}
    ...
    {$aItems=[
        ...
        'text' => "<img src=\"{$oUserCurrent->getProfileAvatarPath(24)}\" alt=\"{$oUserCurrent->getDisplayName()}\" class=\"{$component}-avatar\" /> {$oUserCurrent->getDisplayName()} {$smarty.capture.pro}", 
        'menu' => [
            'items' => $aMenuItems
        ]]
    ]}
    ...
    {component 'nav' hook='fl_userbar_nav' hookParams=[ user => $oUserCurrent ] mods="userbar {$component}" items=$aItems}
