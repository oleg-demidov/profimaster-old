...
<div class="{$component} {cmods name=$component mods=$mods} {$classes}" {cattr list=$attributes} data-id="{$blog->getId()}">
    ...
    {if $isBlogAdmin}
        {component 'actionbar'
            ...
            items=[
                [ 
                    'buttons' => [[ 'url' => "{router page='blog/edit'}{$blog->getId()}", 'text'=>'Редактировать', 'mods'=>'primary'  ]]
                ],
                [
                    'buttons' => [[ 'url' => "{router page='blog/delete'}{$blog->getId()}?security_ls_key={$LIVESTREET_SECURITY_KEY}", 'text'=>'Удалить', 'mods'=>'danger' ]]
                ]
            ]}
    {/if}
</div>